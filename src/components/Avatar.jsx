...
export function ShouryaSharma(props) {
  ...
  const { actions, mixer } = useAnimations([idle.animations[0], wave.animations[0]], group)
  ...
  const handleClick = () => {
    ...
    const duration = actions['Wave']?._clip?.duration || 2
  ...
  return (
    <group {...props} ref={group} dispose={null} onClick={handleClick}>
      <group rotation-x={-Math.PI / 2} rotation-z={Math.PI / 4} scale={1.5}>
